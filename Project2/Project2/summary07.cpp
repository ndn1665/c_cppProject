...
//매개변수의 갯수차이등으로 유도클래스에 기초클래스와 이름이 같은 함수를 정의하여 사용하면, 함수 오버로딩이 되어, 기초클래스의 매개변수 수로 맞춰서 사용하면
//기초클래스의 함수를 사용할 수 있게 되는 함수 오버로딩이 된다.
class Person
{
private:
	int num;
...
int main(void)
{
	...
	Person* pptr[20];
	//manyptr에는 Person객체와 Student객체 둘다 담을 수 있다. 하지만 포인터 연산을 할때는 포인터는 포인터형에 맞춰서 연산을 하므로, manyptr을 사용하여
	//Student객체의 멤버에 접근할 수는 없다.
	Student* sptr[20];
